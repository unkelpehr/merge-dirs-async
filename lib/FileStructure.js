...
class FileStructure {
	...
	async add (filepath, recursive, __source) {
		...
		if (!source.stats.isDirectory()) {
			// await this.fs.ensureDir(path.dirname(dest)); necessary?
			if (write) {
				...
				if (__source) {
					debug(`Copied ${source.relativePath}, from a subsequent source`);
				} else {
					debug(`Copied ${source.relativePath}`);
				}
			}
	...
	async remove (filepath, __originatingChild) {
		...
		if (target.sources.length === 0) {
			...
			this.add(null, null,  target.sources[0]);
		}
